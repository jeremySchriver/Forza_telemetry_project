...
def powerCurvePlotter(fileName, currentGear, carOrdinalID):
    ...
    if len(df) > 0:
        ...
        print("Process aborted due to missing data in " + fileName)
...
def altPowerCurvePlotter(carOrdinalID, numGears):
    ...
    if errorCatch == False:
        ...
        while count <= numGears:
            ...
            plt.scatter(a[key]['roundedCurrentRPM'], a[key]['roundedTorque'], label='Torque vs RPM plot', color='blue')
        ...
        print("Process aborted due to missing data in " + errorFile)
...
def processDataForTorqueVsSpeed(carOrdinalID, numGears):
    ...
    if errorOut == False:
        ...
        validGearDataLEN = len(validGearData)
        shiftPoints = {}

        #Control for situations where number of gears is lower than the number of intersections points found on the chart. This should not happen but occurred in testing on car 2181 with 7 gears and it showing data for gear 11 in telem files.
        if validGearDataLEN < intersection_pointsLEN:
            checkValue = validGearDataLEN
        else:
            checkValue = intersection_pointsLEN

        while count < checkValue:
            ...
            shiftPoints[newKey2] = np.round(mphValue)
            shiftPoints[newKey3] = np.round(torqueValue)
...
def import_module_from_directory(directory, class_name):
    # Save the current sys.path
    prev_sys_path = sys.path.copy()
    print("Previous path = " + str(prev_sys_path))
    try:
        # Add the directory to sys.path temporarily
        sys.path.append(directory)

        # Import the module using importlib
        spec = importlib.util.spec_from_file_location(class_name, directory + "/" + class_name + ".py")
...
#Define variables
telemFileName = "logTelemetry.csv" #Change this if your telemetry file is named differently
captureFile = os.path.join(os.getcwd(), "TelemDataFiles", telemFileName) #Change this if your telemety log file is in a different location than cwd\telemdatafiles
...
if dfSize > 0:
    ...
    if df['CarOrdinalID'].max() != None:
        ...
        print("Grabbed carOrdinalID# " + str(carOrdinalID) + " from the telem log file " + telemFileName)
    else:
        carOrdinalID = input("What is the carOrdinalID for the car?")
        try:
            carOrdinalID = int(carOrdinalID)
        except:
            print("Issue with value entered for carID.")
    #This method is currently not working as telem files are showing data for gear 11 on multiple cars without that gear
    '''if df['Gear'].max() != None:
        numGears = df['Gear'].max()
        print("Grabbed " + str(numGears) + " unique gears from the telem log file " + telemFileName + " for carOrdinalID# " + str(carOrdinalID))
    else:
        numGears = input("How many gears does the car have?")'''
    numGears = input("How many gears does the car have? ")
    try:
        numGears = int(numGears)
    except:
        print("Issue with value entered for carID.")

    carIDDirectory = os.path.join(os.getcwd(), "TelemDataFiles", str(carOrdinalID))

    if os.path.exists(carIDDirectory):
        #Add pause for user input y/delete files previously generated by this class n/continue with warning data may be skewed
        next_input = input("Have you made any performance or tuning updates to the car since the last run of this process? ")
        ...
        if next_input.lower() == "yes" or next_input.lower() == "y" or next_input.lower() == "ys":
            ...
            if next_input.lower() == "yes" or next_input.lower() == "y" or next_input.lower() == "ys":
                ...
                while gearCounter <= numGears:
                    ...
                    powerCurvePlotter(fileName, gearCounter, carOrdinalID)
        ...
        try:
            ...
            # Import the class from the specified directory
            dbModulePath = os.path.join(os.getcwd(),"DBModule")
            mod_name = "carDBmgmt"
            
            carDBmgmt = import_module_from_directory(dbModulePath, mod_name)

            #Ensures telem directory is created for the car
            carDBmgmt.createTelemDataFileDirectoryForCar(carOrdinalID)
            ...
            print("Error with value entered for number of gears")
else: 
